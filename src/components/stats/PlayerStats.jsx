...
function RatingBar({ label, value, baseline, confidence }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs text-gray-300 w-5 text-right tabular-nums">{value}</span>
      {delta !== 0 && confidence > 0.2 && (
        <span className={`text-xs w-7 text-right tabular-nums ${delta > 0 ? 'text-green-400' : 'text-red-400'}`}>
          {delta > 0 ? '+' : ''}{delta}
        </span>
      )}
    </div>
      ...
      <div className="flex gap-1.5 px-3 py-2 overflow-x-auto bg-[var(--color-surface-2)] border-b border-white/5">
        {players.map(p => {
          ...
          return (
            ...
            >
              <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: pos.color }}>
                {p.number}
              </div>
          ...
          <>
                ...
                {profile.hasStats && (
                  <div className="flex gap-1 mt-1">
                    {Object.entries(profile.trends || {}).slice(0, 3).map(([attr, trend]) => (
                      <span key={attr} className="flex items-center gap-0.5 text-xs text-gray-400">
            ...
            {profile.hasStats && narrative && (
              <div className="p-3 rounded-xl bg-[var(--color-surface-2)] border border-white/5">
                <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1">Assessment</div>
            ...
            {/* ── Attribute Ratings ── */}
            <div>
              <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1.5 px-1">
            ...
            {profile.matchHistory?.length >= 2 && (
              <div>
                <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1.5 px-1">Trends</div>
            ...
            {Object.keys(profile.rotationProfile || {}).length >= 3 && (
              <div>
                <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1.5 px-1">By Rotation</div>
            ...
            {profile.tendencies?.length > 0 && (
              <div>
                <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1.5 px-1">Tendencies</div>
                  ...
                  {profile.tendencies.map((t, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 rounded-lg bg-[var(--color-surface-2)] border border-white/5">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-400">{t.evidence}</div>
                      </div>
            ...
            {(profile.strengths?.length > 0 || profile.weaknesses?.length > 0) && (
              <div className="grid grid-cols-2 gap-2">
                {profile.strengths?.length > 0 && (
                  <div>
                    <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1 px-1">Strengths</div>
                    {profile.strengths.map((s, i) => (
                      <div key={i} className="p-2 rounded-lg bg-green-900/15 border border-green-800/20 mb-1">
                        <div className="text-xs text-green-400 font-bold">{s.area}</div>
                        <div className="text-xs text-green-400/70">{s.metric}: {s.value}</div>
                      </div>
                    ))}
                  </div>
                )}
                {profile.weaknesses?.length > 0 && (
                  <div>
                    <div className="text-xs uppercase tracking-wider text-gray-500 font-semibold mb-1 px-1">Growth Areas</div>
                    {profile.weaknesses.map((w, i) => (
                      <div key={i} className="p-2 rounded-lg bg-red-900/15 border border-red-800/20 mb-1">
                        <div className="text-xs text-red-400 font-bold">{w.area}</div>
                        <div className="text-xs text-red-400/70">{w.metric}: {w.value}</div>
                      </div>
                    ))}
                  </div>
                )}
