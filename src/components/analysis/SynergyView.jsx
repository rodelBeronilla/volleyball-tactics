    ...
    <div className="p-3 space-y-3">
      ...
      {view === 'setter' && setterSynergy.map(({ setter, hitters }) => (
        <div key={setter.id}>
          ...
          {hitters.length === 0 ? (
            <div className="text-[10px] text-gray-500 p-2">No attack data available</div>
          ) : (
              ...
              {hitters.map(h => (
                <div key={h.hitterId} className="flex items-center gap-2 p-2 rounded-lg bg-[var(--color-surface-2)] border border-white/5">
                  <div className="flex-1">
                    ...
                    <span className="text-gray-400 text-[10px] ml-1">R{h.rotations.join(',')}</span>
                  </div>
      ...
      {view === 'front' && (
        <div>
          ...
          {frontCombos.length === 0 ? (
            <div className="text-[10px] text-gray-500 p-2">No data</div>
          ) : (
                ...
                <div key={i} className="p-2.5 rounded-lg bg-[var(--color-surface-2)] border border-white/5">
                  ...
                  <div className="flex gap-1">
                    {c.players.map(p => {
                      ...
                      return (
                        <span key={p.id} className="px-2 py-0.5 rounded text-[10px] text-white" style={{ background: pos.color + '44' }}>
                          {p.name}
                        </span>
                      );
                    })}
